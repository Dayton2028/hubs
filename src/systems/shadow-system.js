...
export class ShadowSystem {
  constructor(sceneEl) {
    ...
    this.previousShadowsEnabled = this.dynamicShadowsEnabled = null;
    window.APP.store.addEventListener("statechanged", this.updatePrefs.bind(this));
    this.updatePrefs();
  }

  updatePrefs() {
    this.dynamicShadowsEnabled =
      window.APP.store.state.preferences.enableDynamicShadows === true &&
      window.APP.store.materialQualitySetting !== "low";
    if (this.previousShadowsEnabled !== this.dynamicShadowsEnabled) {
      this.sceneEl.setAttribute("shadow", { enabled: this.dynamicShadowsEnabled });
    ...
    this.previousShadowsEnabled = this.dynamicShadowsEnabled;
  }
